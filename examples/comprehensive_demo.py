...
def demonstrate_game_analysis():
    ...
    for turn in range(20):  # Play 20 turns for analysis
        ...
        print(f"  Tokens - Home: {tokens_home}, Active: {tokens_active}, Finished: {tokens_finished}")
...
def demonstrate_tournament_mode():
    ...
    tournament_strategies = StrategyFactory.get_available_strategies()[:4]  # Use 4 strategies
    ...
    print(f"\nTournament Results:")
    sorted_results = sorted(tournament_results.items(), 
                          key=lambda x: x[1]['wins'], reverse=True)
    
    for rank, (strategy, stats) in enumerate(sorted_results, 1):
        win_rate = (stats['wins'] / stats['games']) * 100 if stats['games'] > 0 else 0
        print(f"  {rank}. {strategy.capitalize()}: {stats['wins']}/{stats['games']} wins ({win_rate:.1f}%)")
