...
class EventHandler:
    ...
    def __init__(self, game_manager, utils, ai_strategies, default_players, show_token_ids):
    ...
    def _ui_init(self, *strats):
        ...
        pil_img = draw_board(self.game_manager.game_state_tokens(game), show_ids=self.show_token_ids)
    ...
    def _ui_steps(self, game, history: list[str], show, pending_dice, human_choice=None):
        ...
        game, desc, tokens, move_opts, waiting = self.game_manager.play_step(game, human_choice, pending_dice)
        history.append(desc)
    ...
    def _ui_run_auto(self, n, delay, game: LudoGame, history: list[str], show: bool):
        ...
        for _ in range(int(n)):
            if self.game_manager.is_human_turn(game):
                ...
                if valid_moves:
                    pil_img = draw_board(self.game_manager.game_state_tokens(game), show_ids=show)
                    html = self.utils.img_to_data_uri(pil_img)
                    player_html = f"<h3 style='color: {current_player.color.value};'>🎯 Current Player: {current_player.color.value.title()}</h3>"
    ...
    def _ui_resume_auto(self, remaining, delay, game: LudoGame, history: list[str], show: bool):
        ...
        if rem <= 0 or game is None:
            ...
            return (
                ...
                "",
                history,
                bool(game and self.game_manager.is_human_turn(game) and not game.game_over),
                player_html,
                "",
                gr.update(visible=False),
                gr.update(visible=False),
    ...
    def _ui_export(self, game: LudoGame):
        ...
        state_dict = {
            ...
            "tokens": {k: [v.to_dict() for v in vs] for k, vs in self.game_manager.game_state_tokens(game).items()},
