...
class UIBuilder:
    ...
    def _build_play_game_tab(self, game_state, move_history, stats_state, waiting_for_human, human_move_options, pending_dice, selected_token_id, auto_steps_remaining, auto_delay_state):
        ...
        # Event Handlers
        init_btn.click(
            self.handler._ui_init, strategy_inputs,
            [
                game_state,
                board_plot,
                log,
        ...
        step_btn.click(
            self.handler._ui_steps, [game_state, move_history, show_ids, pending_dice],
            [
                game_state,
                board_plot,
                log,
            ...
            + move_buttons
            + [human_move_options, pending_dice, selected_token_id, auto_steps_remaining, auto_delay_state]
        ).then(self.handler._ui_update_stats, [stats_state, game_state], [stats_state]).then(lambda s: s, [stats_state], [stats_display])
        ...
        for i, btn in enumerate(move_buttons):
            btn.click(
                ...
                self.handler._ui_make_human_move, [selected_token_id, game_state, move_history, show_ids, human_move_options, pending_dice, auto_steps_remaining, auto_delay_state],
                [
                    game_state,
                    board_plot,
                    log,
                ...
                + move_buttons
                + [human_move_options, pending_dice, selected_token_id, auto_steps_remaining, auto_delay_state]
            ).then(self.handler._ui_update_stats, [stats_state, game_state], [stats_state]).then(lambda s: s, [stats_state], [stats_display]).then(
        ...
        run_auto_btn.click(
            self.handler._ui_run_auto, [auto_steps_n, auto_delay, game_state, move_history, show_ids],
            [
                game_state,
                board_plot,
                log,
    ...
    def _build_simulation_tab(self):
        ...
        bulk_run_btn.click(self.handler._ui_run_bulk, [bulk_games] + sim_strat_inputs, [bulk_results])