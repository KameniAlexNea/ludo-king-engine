...
class CautiousStrategy(BaseStrategy):
    ...
    def choose_move(self, movable_tokens: List[Token], dice_roll: int, game_state: dict) -> Optional[Token]:
        ...
        safe_moves = [t for t in movable_tokens if self.is_move_safe(t, dice_roll, game_state)]
...
class ProbabilisticStrategy(BaseStrategy):
    ...
    def calculate_expected_value(self, token: Token, dice_roll: int, game_state: dict) -> float:
    ...
    def calculate_capture_risk(self, token: Token, dice_roll: int, game_state: dict) -> float:
