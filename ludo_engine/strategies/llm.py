...
Analyze the current game state and choose the best move."""
    ...
    def choose_move(self, movable_tokens: List[Token], dice_roll: int, game_state: dict) -> Optional[Token]:
        ...
        game_description = self._format_game_state(movable_tokens, dice_roll, game_state)
    ...
    def _format_game_state(self, movable_tokens: List[Token], dice_roll: int, game_state: dict) -> str:
            ...
            for token_data in tokens:
                if token_data['color'] != self.player.color:
                    ...
                    if steps >= 50:  # Close to finishing
                        opponent_info.append(f"{token_data['color']} token close to finish")
    ...
    def _parse_llm_decision(self, llm_response: str, movable_tokens: List[Token]) -> Optional[Token]:
        """Parse LLM response to select token."""
        try:
