...
def analyze_game_results(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    analysis = {
        ...
        'efficiency': results['total_moves'] / results['turns_played'] if results['turns_played'] > 0 else 0,
        'player_performance': {},
        'strategy_insights': {}
    }
    ...
    for player_stats in results['player_stats']:
        ...
        analysis['player_performance'][color] = {
            ...
            'capture_efficiency': player_stats['tokens_captured'] / max(player_stats['total_moves'], 1),
            'six_frequency': player_stats['sixes_rolled'] / max(player_stats['total_moves'], 1),
            'moves_per_token_finished': player_stats['total_moves'] / max(player_stats['tokens_finished'], 1)
        }
    
    return analysis


def run_strategy_tournament(strategies: List[str], rounds: int = 10, games_per_round: int = 5) -> Dict[str, Any]:
    """
    ...
    for strategy in strategies:
        ...
        if stats['games_played'] > 0:
            stats['avg_tokens_per_game'] = stats['total_tokens_finished'] / stats['games_played']
            stats['avg_captures_per_game'] = stats['total_captures'] / stats['games_played']
...
def compare_strategies(strategy1: str, strategy2: str, games: int = 50) -> Dict[str, Any]:
    """
...
def calculate_strategy_elo(tournament_results: Dict[str, Any], initial_elo: int = 1200) -> Dict[str, int]:
    """
